...
ENT.NPCFaction = NPC_FACTION_SPORE
ENT.iClass = CLASS_SPORECARRIER
	...
	if(event == "mattack") then
		...
		if(self.UsePoison) then
			fcOnHit = function(ent,dmginfo)
				if(ent:IsPlayer() && math.random(1,4) <= 3) then self:Poison(ent) end
			end
		end
...
function ENT:SelectDefaultSchedule()
	if(IsValid(self.m_lastRagdoll)) then
		...
		if(dist <= 40) then self:SLVPlayActivity(ACT_IDLE_STIMULATED)
		...
		if(IsValid(self.m_entMound)) then
			if(self.m_nextUnburrow) then
				if(CurTime() >= self.m_nextUnburrow) then
					...
					self:slvSetHealth(math.min(hp +20),hpMax)
					self:SLVPlayActivity(ACT_CLIMB_UP)
				end
				return
			end
			...
			local posSelf = self:GetPos()
			local dist = posSelf:Distance(pos)
			if(dist <= 80) then self:SLVPlayActivity(ACT_CLIMB_DOWN)
